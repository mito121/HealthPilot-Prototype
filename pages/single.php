...
<div ng-if="getUserRoleId()==1" class="container">
  <div class="row">
    <div class="col-9">
      </div>
    <div class="col-3">
      <button type="button" class="btn btn-sm btn-warning" style="border-top-left-radius:0; border-top-right-radius:0" data-toggle="modal" data-target="#modal_edit_inst">Ret behandler data</button>
    </div>
  </div>
</div>
...
<div class="container p-5 single" ng-controller="singleCtrl" ng-init="show_single_inst(); currentUserRating(); currentUserRatingText(); getRatings(); getAvg(); getInst_avg_scores(); getComments(); get_avg_total_score(); getPrices(); currentNumRatings()">
    ...
    <div class="row mb-5">
      <div class="col-7 p-0 pr-4"><!-- LEFT COL -->
        ...
        <div class="p-3 mt-3 inst-info single-address hp-box">
          ...
          <br>
          <p><i class="fa fa-adr"></i>{{i_street}}, {{i_zip}} {{c_name}}</p>

          <!-- map -->
          <div class="mt-3">
            <div map-lazy-load="https://maps.google.com/maps/api/js">
              <map center="[{{i_lat}}, {{i_lng}}]" trigger-resize="true">
                ...
                <marker position="[{{i_lat}}, {{i_lng}}]" title="{{i_name}}, {{i_street}}, {{i_zip}} {{c_name}}"></marker>              
              </map>
            </div>
          </div>
        </div>
        ...
        <div  ng-if="i_web!=''" class="col-12 mt-3 p-3 web inst-info hp-box">
            <a ng-href="https://{{i_web}}" target="_blank"><img src='http://image.thum.io/get/https://{{i_web}}' class='embed-responsive' title="Gå til behandlers hjemmeside"></a>
        </div>
        ...
        <!-- ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////-->
        <!-- MODAL -->
        <div class="modal" id="modal_edit_inst">
          <div class="modal-dialog">
            <div class="modal-content">

              <!-- Modal Header -->
              <div class="modal-header">
                <h4 class="modal-title">Ret behandler</h4>
              ...
              <div class="modal-body" ng-init="practiceOptions(); cityOptions(); ownerOptions()">
                ...
                <form ng-submit="update_inst(inst_id, new_name, new_practice.id, new_street, new_zip.zip_id, new_tel, new_email, new_web, new_owner.id)">
                  <label>Navn  (<a href="https://www.google.com/search?q={{i_name}}" target="_blank">Google</a>)</label>
                  <input type="text" ng-model="new_name" class="form-control">
                  
                  <label>Praksis kategori</label>                
                  <select class="form-control selectpicker" id="new_practice" ng-model="new_practice" ng-options="p as p.name for p in optionPractices track by p.id" required>
                  </select>

                  <label>Adresse  (<a href="https://118.dk/search/go?where={{i_street}}, {{i_zip}} {{c_name}}" target="_blank">TDC</a>)</label>
                  <input type="text" class="form-control" ng-model="new_street" required>

                  <label>By</label>
                  <select class="form-control selectpicker" id="new_zip" ng-model="new_zip" ng-options="c as c.city for c in optionCities track by c.zip_id">
                  </select>

                  <label>Telefon  (<a href="https://118.dk/search/go?what={{i_tel}}" target="_blank">TDC</a>)</label>
                  ...
                  <select class="form-control selectpicker" id="new_owner" ng-model="new_owner" ng-options="o as o.owner for o in optionOwners track by o.id">
                  </select>                  
                  <br>
                ...
                <div class="my-3 alert-success text-center">{{update_msg}}</div>
              </div>
              <!-- Modal footer -->
              <div class="modal-footer">
                <form ng-submit="delete_inst(inst_id)">
                  
                  <input type="submit" class="btn btn-danger btn-sm" value="Slet behandler">
