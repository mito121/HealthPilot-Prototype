<div class="container my-5" ng-controller="mainCtrl">
    ...
    <div class="row py-3">
        ...
        <div class="col col-lg col-12 bg-white p-3 m-3">
            <h5><strong>Tilføj behandler</strong></h5>
            <p style="height:82px;">&nbsp;</p>
            <form ng-submit="create_inst()">
                <label>Navn</label>
                <input type="text" class="form-control" ng-model="name" required>

                <div ng-init="practiceOptions()">
                    <label>Praksis kategori</label>
                    <select class="form-control selectpicker" ng-model="practice_id" ng-options="p as p.name for p in optionPractices track by p.id" required>
                        <option value="">Vælg..</option>
                    </select>
                </div>

                <label>Adresse</label>
                <input type="text" class="form-control" ng-model="street" required>

                <label>By</label>
                <div ng-init="cityOptions()">
                    <select class="form-control selectpicker" ng-model="zip_id" ng-options="c as c.city for c in optionCities track by c.zip_id" required>
                        <option value="">Vælg..</option>
                    </select>
                </div>
                ...
                <input type="text" class="form-control" ng-model="web">

                <label>Ejerskab</label>
                <div ng-init="ownerOptions()">
                    <select class="form-control selectpicker" ng-model="owner_id" ng-options="o as o.owner for o in optionOwners track by o.id" required>
                        <option value="">Vælg..</option>
                    </select>
                </div>

                <br>
                <button type="submit" class="btn btn-primary">Tilføj behandler</button>
                {{response}}
            </form>
        ...
        <div class="col col-lg col-12 p-3 m-3 bg-white">	
            ...
            <!-- set LatLong -->
            <button ng-click="setLatLng()">Geocode</button>
            <br>{{geocodeResponse}}
            <!-- Status -->
            <div ng-show="geo_status" class="p-5">GEOCODING IN PROGRESS... Don't close browser.</div>
